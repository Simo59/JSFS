...
module.exports.createItem= async (req,res)=>{
 ...
 try{
 const availbleItems=await Item.find({disponible:true});
 ...
 const RentedItems=await Item.find({disponible:false});
...
module.exports.getRentedItemsByMe= async (req,res)=>{
 ...
 const decoded = jwt.verify(token, jwtConfig.SECRET_TOKEN);
 const userId = decoded.id; 
 const RentedItemsbyme=await Item.find({disponible:false,emprentePar:userId});
...
module.exports.getRentedItemsByOthers= async (req,res)=>{
 ...
 const decoded = jwt.verify(token, jwtConfig.SECRET_TOKEN);
 const userId = decoded.id; 
 const RentedItemsothers=await Item.find({disponible:false ,emprunteur:{$ne:userId}});
...
module.exports.upadteItem= async (req,res)=>{
 ...
 const userId = decoded.id; 
 const Item= await Item.findByIdAnd(userId,{disponible:false,empruntePar:userId}, { new : true });
...
module.exports.rent= async (req,res)=>{
 ...
 const ID=req.params.torent;
    console.log(ID);
 const token = req.cookies.token;
 const decoded = jwt.verify(token, jwtConfig.SECRET_TOKEN);
 
 try{
 const updated= await Item.findByIdAndUpdate(ID,{disponible:false, empruntePar:decoded.id});
 console.log(updated);
...
module.exports.free=async(req,res)=>{
 ...
 try{
 const updated= await Item.findByIdAndUpdate(ID,{disponible:true} );
 console.log(updated);
 res.status(201).json(updated);
 }
 catch(err){
